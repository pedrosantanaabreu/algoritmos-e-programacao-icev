'''
Classe responsável pela exibição da interface gráfica
'''
...
class Interfaces:
    ...
    def imprimir_menu_depositar(agencia : str = '', conta : str = '', valor : str = ''):
        ...
        if valor == '':
            if conta != '':
                _, _, cpf, saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(agencia=agencia, conta=conta)
                nome = Dados.obter_informacoes_pessoais_por_cpf(cpf)
    ...
    def imprimir_menu_editando_cliente(cliente_antigo : Cliente, cliente_novo : Cliente, opcao):
        ...
        if cliente_antigo.cpf == '':
            ...
            if opcao == 1:
                cliente_antigo.nome = Dados.obter_informacoes_pessoais_por_cpf(Utilitarios.formatar_cpf(cliente_antigo.cpf))
                div_esquerda.split_column(
    ...
    def imprimir_menu_adicionar_conta(agencia='', conta='', cpf='', saldo=''):
        ...
        if pagina < 4:
            ...
            cliente.nome = Dados.obter_informacoes_pessoais_por_cpf(cliente.cpf)
    ...
    def imprimir_menu_excluir_cliente(cpf=''):
        ...
        if cpf == '':
            ...
            nome = Dados.obter_informacoes_pessoais_por_cpf(Utilitarios.formatar_cpf(cpf))
            try:
                agencia, conta, cpf, saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(cpf=cpf)
            except:
    ...
    def imprimir_menu_excluir_conta(conta : Conta):
        ...
        if pagina < 3:
            ...
            _, _, _cpf, _ = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(agencia=conta.agencia, conta=conta.conta)
