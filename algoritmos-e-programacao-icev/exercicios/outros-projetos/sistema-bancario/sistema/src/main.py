'''
Classe responsável pelo funcionamento geral da aplicação
'''
...
class Main:
        ...
        while pagina > 0:
            ...
            if pagina == 1:
                ...
                if cliente_antigo.cpf == 1:
                    ...
                    if Dados.verificar_se_cliente_existe(cliente_antigo):
                        pagina += 1
                        cliente_antigo.cpf = Utilitarios.formatar_cpf(cliente_antigo.cpf)
                        cliente_antigo.nome = Dados.obter_informacoes_pessoais_por_cpf(cliente_antigo.cpf)
                    else:
                        cliente_antigo.cpf = ''
                        cliente_antigo.nome = ''
                        Utilitarios.limpar_terminal()
                ...
                if opcao == 1:
                    ...
                    if cliente_novo.cpf == 1:
                        ...
                        if Utilitarios.formatar_cpf(cliente_antigo.cpf) == Utilitarios.formatar_cpf(cliente_novo.cpf):
                            ...
                            if Dados.verificar_se_cliente_existe(cliente_novo):
                                Utilitarios.limpar_terminal()
                    ...
                    if int(confirmacao) == 1:
                        ...
                        if cliente_novo.cpf != cliente_antigo.cpf:
                            while Dados.verificar_cpf_cadastrado_em_conta(cliente_antigo.cpf):
                                _agencia_antiga, _conta_antiga, _cpf_antigo, _saldo_antigo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(cpf=cliente_antigo.cpf)
    ...
    def __editar_conta(cls):
        ...
        while pagina > 0:
            ...
            if pagina == 1:
                ...
                __agencia, __conta, __cpf, __saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(agencia=conta_antiga.agencia, conta=conta_antiga.conta)
                ...
                if int(item_editar) == 1:
                    ...
                    if conta_nova.agencia == 1:
                        ...
                        if conta_nova.agencia == conta_antiga.agencia:
                            ...
                            try:
                                _agencia, _conta, _cpf, _saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(agencia=conta_nova.agencia, conta=conta_antiga.conta)
                                conta_nova = Conta()
                                Utilitarios.limpar_terminal()
                    ...
                    if conta_nova.conta == 1:
                        ...
                        if conta_antiga.conta == conta_nova.conta:
                            ...
                            try:
                                _agencia, _conta, _cpf, _saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(agencia=conta_antiga.agencia, conta=conta_nova.conta)
                                conta_nova = Conta()
                                Utilitarios.limpar_terminal()
                                Interfaces.imprimir_menu_mensagem_de_erro(f'Conta já cadastrada.')
                            except:
                    ...
                    if conta_nova.cpf == 1:
                        ...
                        _agencia, _conta, _cpf, _saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(agencia=conta_antiga.agencia, conta=conta_antiga.conta)
                        if _cpf == conta_nova.cpf:
                            ...
                            if Dados.verificar_se_cliente_existe(cliente):
                                conta_nova.saldo = conta_antiga.saldo
                                conta_nova.conta = conta_antiga.conta
                                conta_nova.agencia = conta_antiga.agencia
                                pagina += 1
        ...
        while pagina > 0:
            ...
            if pagina == 1:
                ...
                if cpf_cliente == 1:
                    ...
                    if Dados.verificar_se_cliente_existe(cliente):
                        cliente.nome = Dados.obter_informacoes_pessoais_por_cpf(cpf_cliente)
                        pagina += 1
                    else:
                        cliente.cpf = ''
                        Utilitarios.limpar_terminal()
                ...
                if Validadores.validar_opcao_menu(opcao_menu_excluir, 2):
                    if int(opcao_menu_excluir) == 1:
                        while Dados.verificar_cpf_cadastrado_em_conta(Utilitarios.formatar_cpf(cliente.cpf)):
                            agencia, conta, cpf, saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(cpf=cliente.cpf)
                            conta = Conta(agencia, conta, cpf, saldo)
                            Dados.deletar_conta_no_banco_de_dados(conta)

                        Dados.deletar_cliente_no_banco_de_dados(cliente)
                        Dados.deletar_linhas_em_branco('clientes')
                        Dados.deletar_linhas_em_branco('contas')
                    ...
                    Utilitarios.limpar_terminal()
                    Interfaces.imprimir_menu_mensagem_de_erro(f'O valor "{opcao_menu_excluir}" é inválido.') 
            else:
                break
        ...
        while pagina > 0:
            ...
            if pagina == 1:
                ...
                if conta.conta == 1:
                    ...
                    if Dados.verificar_se_conta_existe(conta):
                        pagina += 1
                    else:
                        Utilitarios.limpar_terminal()
                ...
                if Validadores.validar_opcao_menu(confirmacao_adicionar_conta, 2):
                    if int(confirmacao_adicionar_conta) == 1:
                        _, _, cpf, saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(agencia=conta.agencia, conta=conta.conta)
                        conta.cpf = cpf
                        conta.saldo = saldo
                    ...
                    Utilitarios.limpar_terminal()
                    Interfaces.imprimir_menu_mensagem_de_erro(f'O valor "{confirmacao_adicionar_conta}" é inválido.') 
            else:
                break
        ...
        while pagina > 0:
            ...
            if pagina == 1:
                ...
                if Validadores.validar_opcao_menu(confirmacao_adicionar_cliente, 2):
                    ...
                    Utilitarios.limpar_terminal()
                    Interfaces.imprimir_menu_mensagem_de_erro(f'O valor "{confirmacao_adicionar_cliente}" é inválido.') 
            else:
                break
        ...
        while pagina > 0:
            ...
            if pagina == 1:
                ...
                if conta_cliente == 1:
                    ...
                    try:
                        _agencia, _conta, _cpf, _saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(agencia=conta.agencia, conta=conta_cliente)
                        conta_verificar = Conta(_agencia, _conta, _cpf, _saldo)
                        Utilitarios.limpar_terminal()
                        Interfaces.imprimir_menu_mensagem_de_erro(f'Conta já cadastrada.')
                        conta.conta = ''   
                    except:
    ...
    def __adicionar_conta_banco(conta : Conta):
        Dados.adicionar_conta_no_banco_de_dados(conta)  
    ...
    def __adicionar_cliente_no_banco(cliente : Cliente):
        Dados.adicionar_cliente_no_banco_de_dados(cliente)
        ...
        while pagina > 0:
            ...
            if pagina == 1:
                ...
                if conta_cliente.conta == 1:
                    ...
                    if Dados.verificar_se_conta_existe(conta_cliente):
                        pagina += 1
                    else:
                        conta_cliente.conta = ''
                        Utilitarios.limpar_terminal()
                ...
                _, _, _, __saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(conta_cliente.agencia, conta_cliente.conta)
                ...
                if Validadores.validar_opcao_menu(confirmacao, 2):
                    if int(confirmacao) == 1:
                        _agencia, _conta, _cpf, _saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(conta_cliente.agencia, conta_cliente.conta)
                        ...
                        nome = Dados.obter_informacoes_pessoais_por_cpf(_cpf)
                        ...
                        Dados.deletar_linhas_em_branco('movimentacoes')
        ...
        while pagina > 0:
            ...
            if pagina == 1:
                ...
                if conta_cliente_remetente.conta == 1:
                    ...
                    if Dados.verificar_se_conta_existe(conta_cliente_remetente):
                        pagina += 1
                    else:
                        conta_cliente_remetente.conta = ''
                        Utilitarios.limpar_terminal()
                ...
                if valor == 1:
                    ...
                    _, _, _, __saldo = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(conta_cliente_remetente.agencia, conta_cliente_remetente.conta)
                    if float(valor) > float(__saldo):
                        Utilitarios.limpar_terminal()
                        Interfaces.imprimir_menu_mensagem_de_erro(f'O valor R$ "{float(valor):.2f}" é maior que seu saldo R$ "{float(__saldo):.2f}".')
                        valor = ''
                ...
                if conta_cliente_destinatario.conta == 1:
                    ...
                    if Dados.verificar_se_conta_existe(conta_cliente_destinatario):
                ...
                if Validadores.validar_opcao_menu(confirmacao, 2):
                    if int(confirmacao) == 1:
                        _agencia_remetente, _conta_remetente, _cpf_remetente, _saldo_remetente = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(conta_cliente_remetente.agencia, conta_cliente_remetente.conta)
                        _agencia_destinatario, _conta_destinatario, _cpf_destinatario, _saldo_destinatario = Dados.obter_informacoes_bancarias_por_agencia_ou_cpf(conta_cliente_destinatario.agencia, conta_cliente_destinatario.conta)
                        ...
                        nome_remetente = Dados.obter_informacoes_pessoais_por_cpf(_cpf_remetente)
                        nome_destinatario = Dados.obter_informacoes_pessoais_por_cpf(_cpf_destinatario)
    ...
    def __escrever_movimentacao(remetente : Cliente, destinatario : Cliente, acao, valor, data=''):
        ...
        Dados.adicionar_movimentacao_no_banco_de_dados(movimentacao)
